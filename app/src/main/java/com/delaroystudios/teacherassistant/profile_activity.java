...
public class profile_activity extends AppCompatActivity{
    ...
    public void find(View view) {
        ...
        //IF want to search by Id

       /* String quroll = "SELECT * FROM STUDENT WHERE roll = '" + editText.getText().toString() + "'";
        Cursor cursorRoll = AppBase.handler.execQuery(quroll);
        if(cursorRoll==null||cursorRoll.getCount()==0)
        {
            Toast.makeText(getBaseContext(),"No Schedules Available",Toast.LENGTH_LONG).show();
        }else
        {
            cursorRoll.moveToFirst();
            while(!cursorRoll.isAfterLast())
            {
                regNo = (cursorRoll.getString(2));
                cursorRoll.moveToNext();
            }
        }
        String reg = regNo;
*/
       // Search By resgister No
        ...
        }else
        {
            ...
            String q = "SELECT * FROM ATTENDANCE WHERE register = '" + reg.toUpperCase() + "'";
